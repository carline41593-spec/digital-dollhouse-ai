...
export default function ImageEditing() {
  ...
  const generate = async () => {
    ...
    const base64 = mainImage.split(",")[1];
    ...
    <div className="min-h-screen bg-black text-white p-12">
      ...
      <div className="max-w-7xl mx-auto grid lg:grid-cols-3 gap-12">
        <div className="space-y-6">
          <h3 className="text-3xl font-bold">Your Face</h3>
          {!mainImage ? (
        ...
        <div className="space-y-6">
          <textarea
            ...
            placeholder="e.g. cyberpunk neon makeup, beach sunset, luxury fashion..."
          ...
          >
            {loading ? "Transforming..." : "Generate Realistic Edit"}
          </button>
        </div>

        <div>
          <h3 className="text-3xl font-bold mb-6">Result</h3>
          {loading && <div className="h-96 bg-gray-900/50 rounded-3xl animate-pulse flex items-center justify-center text-3xl">Working...</div>}
          {result && (
            <div className="space-y-6">
              <img src={result} alt="Result" className="w-full rounded-3xl shadow-2xl" />
              <a href={result} download="dc-realistic-edit.jpg" className="block text-center bg-gradient-to-r from-pink-600 to-purple-600 py-6 rounded-full text-2xl font-bold">
                Download
              </a>
            </div>
          )}
